...
-- ===============================================================================
-- 2. Creating procedure for SILVER (Transforming & Cleaning Data)
-- ===============================================================================
...
BEGIN
    ...
    RAISE NOTICE '   ++ silver.crm_cust_info: %s', EXTRACT(EPOCH FROM (v_end_time - v_start_time));

    -- silver.crm_prd_info
    v_start_time := CURRENT_TIMESTAMP;
    TRUNCATE TABLE silver.crm_prd_info;
    INSERT INTO silver.crm_prd_info (prd_id, cat_id, prd_key, prd_nm, prd_cost, prd_line, prd_start_dt, prd_end_dt)
    SELECT prd_id, REPLACE(SUBSTRING(prd_key FROM 1 FOR 5), '-', '_'), SUBSTRING(prd_key FROM 7), prd_nm, COALESCE(prd_cost, 0),
    ...
    RAISE NOTICE '   ++ silver.crm_prd_info: %s', EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - v_start_time));
    ...
    RAISE NOTICE '   ++ silver.crm_sales_details: %s', EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - v_start_time));

    -- 2. ERP Transformations
    RAISE NOTICE '>> Processing ERP Tables...';

    -- silver.erp_cust_az12
    TRUNCATE TABLE silver.erp_cust_az12;
    INSERT INTO silver.erp_cust_az12 (cid, bdate, gen)
    SELECT CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid FROM 4) ELSE cid END,
    ...
    RAISE NOTICE '==========================================';
    RAISE NOTICE '   SUCCESS: TOTAL SILVER LOAD TIME: % seconds', EXTRACT(EPOCH FROM (v_batch_end_time - v_batch_start_time));
    RAISE NOTICE '==========================================';

    -- Clearing and loading API data 
    TRUNCATE TABLE silver.api_exchange_rates;
    
    INSERT INTO silver.api_exchange_rates (base_currency, target_currency, rate)
    SELECT 
        base_currency,
        target_currency,
        rate
    FROM bronze.api_exchange_rates;

EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'ERROR IN SILVER LOAD: %', SQLERRM;
END;
$$;
